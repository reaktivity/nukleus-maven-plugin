...
public final class String16FlyweightGenerator extends ClassSpecGenerator
{
    ...
    private MethodSpec limitMethod()
    {
        return methodBuilder("limit")
                ...
                .addModifiers(PUBLIC)
                .returns(int.class)
                .addStatement("int length = length0()")
                .beginControlFlow("if (length == 65535)")
                .addStatement("length = 0")
                .endControlFlow()
                .addStatement("return maxLimit() == offset() ? offset() : offset() + FIELD_SIZE_LENGTH + length")
                .build();
    }
    ...
    private MethodSpec asStringMethod()
    {
        return methodBuilder("asString")
                ...
                .endControlFlow()
                .beginControlFlow("if (length0() == 65535)")
                .addStatement("return null")
                .endControlFlow()
    ...
    private static final class BuilderClassBuilder
    {
        ...
        private MethodSpec setDirectBufferMethod()
        {
            return methodBuilder("set")
                    ...
                    .addStatement("throw new IllegalArgumentException("+
                            "String.format(\"length '%d' too big for String field (length < 65535)\", length))")
                    .endControlFlow()
        ...
        private MethodSpec setStringMethod()
        {
            return methodBuilder("set")
                    ...
                    .addStatement("byte[] charBytes")
                    .addStatement("int length")
                    .beginControlFlow("if (value == null)")
                    .addStatement("charBytes = new byte[0]")
                    .addStatement("length = -1")
                    .nextControlFlow("else")
                    .addStatement("charBytes = value.getBytes(charset)")
                    .addStatement("length = charBytes.length")
                    .endControlFlow()
                    .beginControlFlow("if (length >= 65535)")
                    .addStatement("throw new IllegalArgumentException("+
                            "String.format(\"Value '%s' too big for String field (length < 65535)\", value))")
                    .endControlFlow()
